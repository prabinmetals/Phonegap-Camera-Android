...
public class GeoBroker extends CordovaPlugin {
		...
		if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)
			...
			if (action.equals("getLocation")) {
				...
				Location last = this.locationManager
						.getLastKnownLocation((enableHighAccuracy ? LocationManager.GPS_PROVIDER
				...
				if (last != null
					...
					this.getCurrentLocation(callbackContext,
							enableHighAccuracy, args.optInt(2, 60000));
				}
	...
	private void getCurrentLocation(CallbackContext callbackContext,
			boolean enableHighAccuracy, int timeout) {
	...
	public boolean isGlobalListener(CordovaLocationListener listener) {
		if (gpsListener != null && networkListener != null) {
			return gpsListener.equals(listener)
					|| networkListener.equals(listener);
		} else
			return false;
	}
}
